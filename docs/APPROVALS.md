...
| --- | --- | --- |
| **б** | 2026-08-07 | The last Ukrainian letter, and the only one with no capital to derive from -- Б is a different drawing -- and no Latin lowercase of its shape. **The bowl is о** whole: the panel puts б's bowl at 1.000 of о's width, middle half 0.988-1.000, tighter than anything б offers against b or the six. **What decides whether it reads as б at all is where the flag begins** -- `BE_FLAG`, fitted over the 50 faces and strongly sloped, 0.35 of the width at Thin falling to 0.16 at ExtraBold. So the flag is set and **the rise's lean is solved for**: it has to land there and stay tangent to the bowl, two constraints and one free variable. **Tangency is the six's rule** and the one thing the six really donates -- measured against its own bowl the six's rise has its left edge ON the bowl's left edge, within a unit over the upper half at both masters. The turn takes both of the face's own radii, the way г's corner does. Flag reach `BE_REACH` 0.915 of the width; weight the face's own lcBar, so `signature.py` reads it as one of the face's own horizontals; top flat on b's ascender with no overshoot, which is the panel's lower quartile exactly. **Rejected three times by eye before this.** Twice the rise was built at the six's own constant slope, which is the wrong donor: a six never turns, and every panel б swings from a lean of 0.49 to a slope of 2.9-3.7 in its top third, so the silhouette read as a six with a nub. The second of those was caused by a broken probe -- see METHOD F6, a scan walking in from the top edge that found the flag's rounded tip and called it the rise. **To record if approved:** two readings outside the panel, both the face rather than the letter. б is 1.48-1.55 of the x-height where the panel holds 1.42-1.46, but this face's own b is 1.481-1.547 against a panel 1.389-1.530 -- the ascender is tall here, and б sits at b's own top, which is what the panel's other reading asks for. Also: б was taken **out of `audit.py`'s bowls check**, which asks whether a bowl hung off a stem ends where в's lower lobe ends -- the right question for ь and ъ and the wrong one for б, whose bowl is о whole. |
...
**Capitals** — А Б В Г Д Е И Й Н О П Р С Т У Х Ц Ч Ш Щ Ы Ь Э Ю
