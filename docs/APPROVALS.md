...
| --- | --- | --- |
| **б** | 2026-08-09 | **In review — not approved.** Nine constructions were drawn for this letter and none was accepted, so it is no longer drawn: the outline is **Sudo's**, which is under the SIL Open Font License and so may be an outline donor. `scripts/be_from_sudo.py` regenerates `tools/be_donor.py`; the recipe only reads that. **The donor's axis does not reach either master.** Sudo's wall runs from a tenth of the x-height to just under a quarter and this face needs a sixteenth at Thin and three tenths at ExtraBold, so its own axis is extrapolated point for point past both ends. **The weight needs two levers, not one:** the donor does not hold this face's proportion of wall to floor and no single point on its axis can be made to, so the axis solves the WALLS against o's own and the bowl's counter is then squeezed vertically until the FLOOR matches too — squeezing the counter moves only the ink above and below it, so the walls stay where the axis put them. **Fitted:** baseline and x-height anchored on o, so the overshoot is this face's; everything above the x-height stretched on its own to reach the ascender, because Sudo's б stands 1.34 x-heights and this face's lowercase stands 1.50 to 1.57; width fitted to o's; and the terminal squared, because this face cuts 213 of its 242 terminals at exactly 0 or 90 degrees and the donor cuts this one oblique. **Where it sits:** its horizontal reads 1.01 / 1.08 / 1.03 / 1.07 of the face's own across the four weights, against o's 1.05 / 1.07 / — / 1.07 — it tracks о across the whole axis, where в is the light one at 0.89–0.96 and always was. Two quadratic-to-cubic traps are in METHOD: a curvature-fitted conversion gives the two masters different node counts, and pathops answers in quadratics wherever the input was one. Gates at the time of showing: mechanical and interpolation pass, audit clean on both readings, panel ink area every letter inside the range, stroke weight inside at every master, signature 0 findings on both readings. The one red line in `verify.sh` is м at ExtraBold, which is pre-existing and untouched. |
