...
| --- | --- | --- |
| **м** | 2026-08-07 | М's construction at x-height, and М's proportion in every part of it — М is the Latin M and Latin m is three arches, so there is no lowercase counterpart to take. Box, upright weight, diagonal weight and vertex fraction are all read off M at each master. Only the **diagonals** take `crowd3`: the box is М's, so the same width is crossed over two thirds of the height and they lean half again as hard. The uprights do not, and taking it there was the fault the user's eye caught — М's own 0.845 of the stem already is this face's four-stroke reduction, so `crowd3` on top counted it twice and left м at 0.704 of n where п and и sit at 1.000. Built measurement 1.000, 0.877, 0.856, 0.847 of n against М's own 1.000, 0.872, 0.852, 0.845 of H — the panel's ratio between the two cases is 1.000, middle half inside 0.979–1.035. **To record if approved:** the vertex is at М's own fraction where the panel's median puts м's at 0.639 of it. The two cannot both be had — this letter cannot widen, and at the panel's vertex with this upright weight the wedge shuts to six units at Bold. The upright reading is the tight one and the vertex reading's middle half runs 0.308–0.889, so the vertex is what gave. Two earlier vertices were rejected: М's fraction with `crowd3` on both strokes (starved), and the baseline (below every panel face's lower quartile, and it read as standing on its point). |
