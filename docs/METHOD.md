...
`walk` in the retired `be_from_sudo.py`, which generated б, is the worked
example; it is in git history.
...
apart and, as it happens, in the same segment; `be_from_sudo.py` asserted
...
| `be_donor.py`, `de_donor.py`, `ge_donor.py` | б, cursive д and cursive г as **frozen data** — the source now, not an output. The generators that fitted them are retired, below, so the font needs no other face installed | — |
...
They are cited by name in comments across `tools/ve/donor.py`,
`tools/gd_band.py` and `tools/classify.py`, and those
...
| `de_paths.py` | every candidate donor's own stroke laid over this face's o | APPROVALS, the д rows; `tools/de_donor.py` |
...
**The three donor generators went on 2026-09-17**, on the user's word: SUSE
Mono must not depend on another font. Each read its donor from the installed
system fonts and wrote a data file; all three reproduced their data byte for
byte on the day they were retired, so the data is exactly what they last made.
The data is now the source. `donor.py`, their shared plumbing, moved to
`tools/ve/`, where the в probes still use it to read reference faces.

| retired | what it established | where the finding lives now |
| --- | --- | --- |
| `be_from_sudo.py` | б's branch from Sudo, walked onto this face's o, weighed against the panel, per master and per style | `tools/be_donor.py`; APPROVALS, the б rows; F9, F11, F29 |
| `ge_from_lilex.py` | the italic г, Lilex's outline scaled to o's height and weighed to o's wall | `tools/ge_donor.py`; APPROVALS, the г rows; F18 |
| `de_from_lilex.py` | the italic д's arm, Lilex's path stroked at this face's weight and spliced into o | `tools/de_donor.py`; APPROVALS, the д rows; F19, F20 |
...
| `_be_paths` | б's walked contour, in `recipes.py` | `tools/be_donor.py` |
| `DE_SEAM` | the height, in o's heights, of the knot д's seam is cut inside: 1.28 | frozen into `tools/de_donor.py` |
| `DE_SIZE` | д's arm scaled about its root to this bowl: 0.82 | frozen into `tools/de_donor.py` |
...
- **б, and the italic г and д, are frozen against the o of 2026-09-17.**
  Their outlines are data, fitted to o at that date; only д's counter is still
  read live. A redrawn o leaves б's bowl and д's outer on the old o, and no
  gate compares them. Fitting them again means bringing a retired generator
  back from git history, and with it a donor font — ask first. б's landing
  was asserted, not chosen: it falls in the same segment of the bowl at both
  masters, and a generator run against a new o must keep that assertion.
